...
public class BookEntityResourceIT {
     ...
     *
     * @see com.arttha.repository.search.BookEntitySearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createBookEntity() throws Exception {
        ...
        // Validate the BookEntity in Elasticsearch
        verify(mockBookEntitySearchRepository, times(1)).save(testBookEntity);
    }
    ...
    public void createBookEntityWithExistingId() throws Exception {
        ...
        // Validate the BookEntity in Elasticsearch
        verify(mockBookEntitySearchRepository, times(0)).save(bookEntity);
    }
    ...
    public void updateBookEntity() throws Exception {
        ...
        reset(mockBookEntitySearchRepository);
    ...
    public void deleteBookEntity() throws Exception {
        ...
        // Validate the BookEntity in Elasticsearch
        verify(mockBookEntitySearchRepository, times(1)).deleteById(bookEntity.getId());
    }

    @Test
    @Transactional
    public void searchBookEntity() throws Exception {
        // Initialize the database
        bookEntityService.save(bookEntity);
        when(mockBookEntitySearchRepository.search(queryStringQuery("id:" + bookEntity.getId())))
            .thenReturn(Collections.singletonList(bookEntity));
        // Search the bookEntity
        restBookEntityMockMvc.perform(get("/api/_search/book-entities?query=id:" + bookEntity.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(bookEntity.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
            .andExpect(jsonPath("$.[*].publisher").value(hasItem(DEFAULT_PUBLISHER)))
            .andExpect(jsonPath("$.[*].publishedDate").value(hasItem(DEFAULT_PUBLISHED_DATE)))
